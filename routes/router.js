...
function authenticate(passport) {
    ...
    async.waterfall(
      [
        ...
        function (token, done) {
          User.findOne({ email: req.body.email }, function (err, user) {
            ...
            user.resetPasswordExpires =
              Date.now() + keys.passwordExpirationTimeInMills; // 1 hour
        ...
        function (token, user, done) {
          let mailOptions = keys.addKeyValue(
            keys.forgotMailOptions,
            'to',
            user.email
          );
          let replaceHostAndToken = mailOptions['text']
      ...
      function (err, info) {
        if (err)
          req.flash(
            'error',
            'Soomething wrong happened, probably Email sending is disabled.'
          );
  ...
  router.get('/reset/:token', function (req, res) {
    User.findOne(
      ...
      function (err, user) {
        ...
        res.render('reset', { layout: 'pre_signin', token: req.params.token });
      }
    );
  });
      ...
      [
        ...
        function (user, done) {
          ...
          if (disableMailSending && disableMailSending == 'no') {
            ...
            console.warn(
              'Email sending is disabled, Success! Your password has been changed.'
            );
          }
        },
      ],
  ...
  router.post('/register', (req, res, next) => {
    User.create(req.body)
      .then((user) => {
        let regMailOptions = keys.addKeyValue(
          keys.regMailOptions,
          'to',
          user.email
        );
    ...
    Post.create(req.body)
      ...
      .catch((err) => {
        ...
        if (err) {
          req.flash(
            'error',
            'Soomething wrong happened while creating the post!'
          );
          res.redirect('/');
        } else next(err);
      });
